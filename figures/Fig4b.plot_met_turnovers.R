...
dev.off()
#Manual table
data2 <- read.csv('../results/met_precursors_turnovers_allChemicals.txt',sep='\t',stringsAsFactors = TRUE)
...
clusters$internal_ids <- gsub('_','',clusters$id)
# fill cluster column according to match clusters internal ids with results chem
# only keep rows in results that chem is in clusters internal ids
results <- results[results$chem %in% clusters$internal_ids,]
# set the order of the rows in results according to the order of the internal ids in clusters
results <- results[match(clusters$internal_ids, results$chem), ]
...
# only keep rows that index in results chem
metDF <- metDF[rownames(metDF) %in% results$chem,]
# set the order of the rows in results according to the order of the internal ids in clusters
metDF <- metDF[match(results$chem, rownames(metDF)), ]
